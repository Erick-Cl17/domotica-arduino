...
const listarLecturas = async (req, res) => {
    try {
        ...
        if (dias) {
            ...
            fechaLimite.setDate(
                fechaLimite.getDate() - Number(dias)
            );
...
// GET - Obtener la última lectura
const ultimaLectura = async (req, res) => {
    try {
        const lectura = await Lectura
            .query()
        ...
        res.status(500).json({
            error: "Error al obtener la última lectura",
            message: error.message
        });
    }
};
...
// GET - Obtener potencia actual
const potenciaActual = async (req, res) => {
    try {
        ...
        if (!lectura) {
            return res.status(404).json({
                error: "No existen lecturas disponibles"
            });
        }

        const voltaje = Number(lectura.voltaje) || 0;
        const amperaje = Number(lectura.amperaje) || 0;
        ...
        res.status(500).json({
            error: "Error al calcular la potencia",
            message: error.message
        });
    }
};
...
const insertarLectura = async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: "Error al insertar la lectura",
            message: error.message
        });
    }
};
...
const patchLectura = async (req, res) => {
    try {
        ...
        const filasActualizadas = await Lectura
            .query()
            .findById(id)
        ...
        const lecturaActualizada = await Lectura
            .query()
            .findById(id);

        res.status(200).json({
            mensaje: "Lectura actualizada con éxito",
            data: lecturaActualizada
        });
...
const putLectura = async (req, res) => {
    try {
        ...
        res.status(200).json({
            mensaje: "Lectura reemplazada con éxito",
            data: lecturaActualizada
        });
        ...
        res.status(500).json({
            error: error.message,
            mensaje: "Error al reemplazar lectura"
        });
    }
};
...
const eliminarLectura = async (req, res) => {
    try {
        ...
        const filasBorradas = await Lectura
            .query()
            .deleteById(id);
        ...
        res.status(200).json({
            mensaje: `Lectura con ID ${id} eliminada correctamente`
        });
...
// POST - Consultar OpenWeatherMap y guardar lectura
const obtenerYGuardarClima = async (req, res) => {
    try {
        ...
        res.status(201).json({
            mensaje: "Lectura obtenida de la API de clima",
            data: resultado
        });
